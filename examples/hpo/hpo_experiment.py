...
def train(config):
    ...
    if config.opt_hps == "":
        ...
        for hp in opt_hps:
            if isinstance(config.algo_config[hp], list) and not isinstance(
                opt_hps[hp], list
            ):
    ...
    results, metrics = experiment.run_evaluation(
        n_episodes=1, n_steps=None, done_on_max_steps=True
    )
    control_agent.close()
...
if __name__ == "__main__":
    ...
    fac.add_argument(
        "--opt_hps", type=str, default="", help="yaml file as a result of HPO."
    )
    fac.add_argument(
        "--load_study",
        type=bool,
        default=False,
        help="whether to load study from a previous HPO.",
    )
    fac.add_argument(
        "--sampler", type=str, default="TPESampler", help="which sampler to use in HPO."
    )
    # merge config
    config = fac.merge()
