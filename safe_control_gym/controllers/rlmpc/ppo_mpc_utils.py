...
class PPO_MPC_Agent:
    ...
    def __init__(self,
        ...
        # Optimizers.
        # for i in self.ac.actor.parameters():
        #     print(i)
        # print('done')
        # p()
    ...
    def compute_policy_loss(self, batch, batch_th):
        ...
        info = batch['info']
        action_th, dist, logp, nabla_pi_cost, nabla_pi_model, optimal = self.ac.actor.forward_train(obs, act, info)
        # Policy.
        ratio = torch.exp(logp - logp_old)
        clip_adv = torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * adv
        policy_loss = torch.where(optimal > 0.9, -torch.min(ratio * adv, clip_adv), torch.nan).nanmean()
        # Entropy.
        ...
        return policy_loss, entropy_loss, approx_kl, action_th, nabla_pi_cost, nabla_pi_model
            ...
            for batch, batch_th in rollouts.sampler(self.mini_batch_size, device):
                ...
                if iter_count % 3 == 0:
                    (policy_loss, entropy_loss, approx_kl, action_th,
                     nabla_pi_cost, nabla_pi_model) = self.compute_policy_loss(batch, batch_th)
                    ...
                    if (self.target_kl <= 0) or (self.target_kl > 0 and approx_kl <= 1.5 * self.target_kl):
                        ...
                        theta = self.ac.actor.pre_pi_net(batch_th['obs'])
                        ...
                        temp1 @= (theta[:, :self.ac.actor.pi_net.param_dict['l'].shape[0]].unsqueeze(2))
                        ...
                        # temp2 = 0*np.matmul(action_th.grad.unsqueeze(1).numpy(), nabla_pi_model).squeeze(1).mean(axis=0)
                        # grads = {'l': temp1, 'f': temp2}
                        # self.ac.actor.pi_net.param_dict = self.actor_mpc_opt.update_params(
                        #     self.ac.actor.pi_net.param_dict, grads
                        # )
...
class MPCActor(nn.Module):
    ...
    def __init__(self, env, obs_dim, act_dim, hidden_dims, activation, gamma, model, actor_config):
        super().__init__()
        self.pi_net = MPCPolicyFunction(env, gamma, model, **actor_config)
        self.n_learnable_param = 0
        for k in self.pi_net.param_dict.keys():
            self.n_learnable_param += self.pi_net.param_dict[k].shape[0]
        self.pre_pi_net = MLP(obs_dim, self.n_learnable_param, hidden_dims, activation)
    ...
    def forward(self, obs, act=None):
        theta = self.pre_pi_net(torch.FloatTensor(obs))
        if obs.ndim > 1:
            action, info, results_dict, optimal_flag = self.pi_net.select_action_batch(obs, theta.numpy())
        else:
            action, info, results_dict, optimal_flag = self.pi_net.select_action(obs, theta.numpy())
    ...
    def forward_train(self, obs, act, info):
        theta = self.pre_pi_net(torch.FloatTensor(obs))
        action, nabla_pi_cost, nabla_pi_model, optimal_flag = self.pi_net.select_action_batch_train(
            obs, theta.detach().numpy(), info
        )
        action_th = action
        action_th.requires_grad_()
        dist = self.dist_fn(action_th)
        logp_a = dist.log_prob(act)
        return action_th, dist, logp_a, nabla_pi_cost, nabla_pi_model, optimal_flag
...
class MPCPolicyFunction:
    ...
    def setup_optimizer(self):
        ...
        dPi_cost = cs.Function('dPi_cost', [z, fixed_param, cost_param, model_param], [dzdP_cost[: nu, :]])
        dzdP_model = -cs.inv(dRdz) @ dRdP_model
        dPi_model = cs.Function('dPi_model', [z, fixed_param, cost_param, model_param], [dzdP_model[: nu, :]])
    ...
    def select_action(self, obs, theta, info=None, mode='eval'):
        ...
        cost_param = (self.param_dict['l'] + 0.1 * theta[:self.param_dict['l'].shape[0]])[:, None]
        model_param = (self.param_dict['f'] + 0.0 * theta[self.param_dict['l'].shape[0]:])[:, None]
    ...
    def select_action_batch(self, obs_batch, theta):
        solver_dict = self.solver_dict
        solver = solver_dict['solver']
        lbg = solver_dict['lower_bound']
        ubg = solver_dict['upper_bound']
        ...
        for i, obs in enumerate(obs_batch):
            ...
            cost_param = (self.param_dict['l'] + 0.1 * theta[i, :self.param_dict['l'].shape[0]])[:, None]
            model_param = (self.param_dict['f'] + 0.0 * theta[i, self.param_dict['l'].shape[0]:])[:, None]
            ...
            temp = [solver, opt_vars_init, goal_states, fixed_param, cost_param, model_param, lbg, ubg]
        ...
        for data in data_batch:
            eval_data, soln, optimal = data
            solver, opt_vars_init, goal_states, fixed_param, cost_param, model_param, lbg, ubg = eval_data
    ...
    def select_action_batch_train(self, obs_batch, theta, info_batch):
        ...
        optimal_batch = []
        for data in data_batch:
            action, _, nabla_pi_cost, nabla_pi_model, optimal = data
            action_batch.append(action)
...
def _select_action_train(eval_data):
    ...
    if optimal:
        ...
        nabla_pi = dpi(z, fixed_param, cost_param, model_param).full()
        nabla_pi_cost = nabla_pi[:, :cost_param.shape[0]]
    ...
    return action, soln, nabla_pi_cost, nabla_pi_model, optimal
