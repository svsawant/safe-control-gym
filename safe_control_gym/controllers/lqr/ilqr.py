...
class iLQR(BaseController):
    ...
    def select_action(self, obs, info=None, training=False):
        ...
        if training:
            if self.ite_counter == 0:
                action, gains_fb, input_ff = self.calculate_lqr_action(
                    obs, self.traj_step
                )
                ...
                action = (
                    self.gains_fb[self.traj_step].dot(obs)
                    + self.input_ff[:, self.traj_step]
                )
        elif self.gains_fb_best is not None:
            action = (
                self.gains_fb_best[self.traj_step].dot(obs)
                + self.input_ff_best[:, self.traj_step]
            )
        else:
