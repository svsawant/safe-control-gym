...
class PID(BaseController):
        ...
        **kwargs
        ...
        if self.env.NAME != Environment.QUADROTOR:
            raise NotImplementedError(
                "[ERROR] PID not implemented for any system other than Quadrotor (2D and 3D)."
            )
    ...
    def select_action(self, obs, info=None):
        ...
        if self.env.QUAD_TYPE == 2:
            if self.env.TASK == Task.TRAJ_TRACKING:
                target_pos = np.array(
                    [self.reference[step, 0], 0, self.reference[step, 2]]
                )
                target_vel = np.array(
                    [self.reference[step, 1], 0, self.reference[step, 3]]
                )
            ...
            if self.env.TASK == Task.TRAJ_TRACKING:
                ...
                target_pos = np.array(
                    [self.reference[0], self.reference[2], self.reference[4]]
                )
                target_vel = np.array([0, 0, 0])
        ...
        thrust, computed_target_rpy, _ = self._dslPIDPositionControl(
            cur_pos, cur_quat, cur_vel, target_pos, target_rpy, target_vel
        )
        rpm = self._dslPIDAttitudeControl(
            thrust, cur_quat, computed_target_rpy, target_rpy_rates
        )
    ...
    def _dslPIDPositionControl(
        self, cur_pos, cur_quat, cur_vel, target_pos, target_rpy, target_vel
    ):
    ...
    def _dslPIDAttitudeControl(self, thrust, cur_quat, target_euler, target_rpy_rates):
        ...
        rpy_rates_e = (
            target_rpy_rates - (cur_rpy - self.last_rpy) / self.control_timestep
        )
    ...
    def reset(self):
        ...
        self.GRAVITY = (
            self.g * self.model.quad_mass
        )  # The gravitational force (g*M) acting on each drone.
        self.env.reset()
