...
class GPMPC(MPC):
    ...
    def train_gp(
        ...
        if self.parallel:
            likelihood = gpytorch.likelihoods.GaussianLikelihood(
                batch_shape=torch.Size([len(self.target_mask)]),
    ...
    def learn(self, env=None):
        ...
        if self.same_test_initial_state:
            for epoch in range(self.num_epochs):
                test_envs.append(
                    self.env_func(randomized_init=True, seed=self.seed * 111)
                )
        ...
        for episode in range(self.num_train_episodes_per_epoch):
            run_results = self.prior_ctrl.run(
                env=train_envs[0], terminate_run_on_done=self.terminate_train_on_done
            )
        ...
        for test_ep in range(self.num_test_episodes_per_epoch):
            run_results = self.run(
                env=test_envs[0], terminate_run_on_done=self.terminate_test_on_done
            )
        ...
        for epoch in range(1, self.num_epochs):
            ...
            if self.rand_data_selection:
                x_seq, actions, x_next_seq = self.gather_training_samples(
                    train_runs,
                    epoch - 1,
                    self.num_samples,
            ...
            train_inputs, train_outputs = self.preprocess_training_data(
