...
def covMatern52ard(x, z, ell, sf2):
    ...
    return (
        sf2
        * (1 + ca.sqrt(5) * r_over_l + 5 / 3 * r_over_l**2)
        * ca.exp(-ca.sqrt(5) * r_over_l)
    )
...
class ZeroMeanIndependentMultitaskGPModel(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, nx, kernel="RBF"):
...
class GaussianProcessCollection:
    ...
    def get_hyperparameters(self, as_numpy=False):
        ...
        if self.parallel is False:
            ...
            lengthscale = (
                self.gps.model.covar_module.base_kernel.lengthscale.detach().squeeze()
            )
    ...
    def train(
        ...
        if self.parallel is False:
            ...
            for gp_ind, gp in enumerate(self.gp_list):
                ...
                print("#########################################")
                print(
                    f"#      Training GP dimension {self.target_mask[gp_ind]}         #"
                )
                print("#########################################")
            ...
            for i in range(len(learning_rate)):
                assert learning_rate[i] == learning_rate[0], ValueError(
                    "Learning rate must be the same for all GPs."
                )
            for i in range(len(n_train)):
                assert n_train[i] == n_train[0], ValueError(
                    "Training iterations must be the same for all GPs."
                )
            lr = learning_rate[0]
            n_t = n_train[0]
            self.gps.train(
...
class BatchGPModel:
    ...
    def __init__(
        ...
        self.target_mask = target_mask
        self.kernel = kernel
        assert normalize is False, NotImplementedError(
    ...
    def _init_model(self, train_inputs, train_targets):
        ...
        self.model = BatchIndependentMultitaskGPModel(
            train_inputs.unsqueeze(0).repeat(target_dimension, 1, 1),
            train_targets.T,
            self.likelihood,
            kernel=self.kernel,
        )
    ...
    def _compute_GP_covariances(self, train_x):
        ...
        self.gp_K_plus_noise_inv = K_lazy_plus_noise.inv_matmul(
    ...
    def init_with_hyperparam(self, train_inputs, train_targets, path_to_statedict):
        ...
        state_dict = torch.load(
            path_to_statedict, map_location=device, _use_new_zipfile_serialization=True
        )
        ...
        self.model = self.model.to(
            dtype=torch.float64
        )  # needed otherwise loads state_dict as float32
        ...
        while i < n_train and torch.abs(loss - last_loss) > 1e-2:
            with torch.inference_mode():
                ...
                test_output = self.model(
                    test_x.unsqueeze(0).repeat(self.output_dimension, 1, 1)
                )
            ...
            output = self.model(
                train_x.unsqueeze(0).repeat(self.output_dimension, 1, 1)
            )
            loss = -mll(output, train_y.T).sum()
            loss.backward()
            if i % 100 == 0:
                print(
                    "Iter %d/%d - MLL train Loss: %.3f, Posterior Test Loss: %0.3f"
    ...
    def predict(self, x, requires_grad=False, return_pred=True):
        ...
        if requires_grad:
            predictions = self.likelihood(
                self.model(x.unsqueeze(0).repeat(self.output_dimension, 1, 1))
            )
        ...
        for i in range(Ny):
            ...
            y += [
                ca.Function(
                    ...
                    [
                        K_z_ztrain(z=z)["K"]
                        @ self.gp_K_plus_noise_inv[i, :, :].detach().numpy()
                        @ train_targets[:, i]
                    ],
                    ["z"],
                    ["mean"],
    ...
    def _init_model(self, train_inputs, train_targets):
        ...
        if self.model is None:
            self.model = self.model_type(
                train_inputs, train_targets, self.likelihood, self.kernel
            )
        # Extract dimensions for external use.
        self.input_dimension = train_inputs.shape[1]
        self.output_dimension = target_dimension
