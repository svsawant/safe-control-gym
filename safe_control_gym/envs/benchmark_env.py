...
class BenchmarkEnv(gym.Env, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(episode_len_sec, list):
            self.EPISODE_LEN_SEC = episode_len_sec[0]
        else:
            self.EPISODE_LEN_SEC = episode_len_sec
    ...
    def before_reset(self, seed=None):
        ...
        self.current_physical_action = (
            None  # current_raw_action unnormalized if it was normalized
        )
        self.current_noisy_physical_action = (
            None  # current_physical_action with noise added
        )
    ...
    def after_reset(self, obs, info):
        ...
        info["current_step"] = 0
        if self.constraints is not None and not (
