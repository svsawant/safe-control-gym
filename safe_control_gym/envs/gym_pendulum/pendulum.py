...
class Pendulum(BenchmarkEnv):
    ...
    URDF_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'pendulum_template.urdf')
    ...
    def __init__(self,
        ...
        if init_state is None:
            ...
            raise ValueError('[ERROR] in Pendulum.__init__(), init_state incorrect format.')
        ...
        EFFECTIVE_POLE_LENGTH, POLE_MASS = self._parse_urdf_parameters(self.URDF_PATH)
        ...
        if inertial_prop is None:
            ...
            raise ValueError('[ERROR] in Pendulum.__init__(), inertial_prop incorrect format.')

        # Create X_GOAL and U_GOAL references for the assigned task.
        self.U_GOAL = np.zeros(1)
        if self.TASK == Task.STABILIZATION:
            self.X_GOAL = np.hstack([self.TASK_INFO['stabilization_goal'][0], 0.])  # x = {theta, theta_dot}.
    ...
    def reset(self, seed=None, init_state=None):
        ...
        if self.RANDOMIZED_INERTIAL_PROP:
            ...
            if any(phy_quantity < 0 for phy_quantity in prop_values.values()):
                raise ValueError('[ERROR] in Pendulum.reset(), negative randomized inertial properties.')
        ...
        self.override_path = os.path.join(self.output_dir, f'pid-{os.getpid()}_id-{self.idx}_pendulum.urdf')
        ...
        for joint_idx in [0]:  # cart-to-pole joint.
            p.setJointMotorControl2(self.PENDULUM_ID, jointIndex=joint_idx, controlMode=p.VELOCITY_CONTROL, force=0, physicsClientId=self.PYB_CLIENT)
        ...
        p.resetJointState(
            ...
            jointIndex=0,  # Cart-to-pole joints.
    ...
    def _get_observation(self):
        ...
        Returns:
            obs (ndarray): The state (theta, theta_dot) of the pendulum.
        '''
        if not np.array_equal(self.state, np.clip(self.state, self.state_space.low, self.state_space.high)) and self.VERBOSE:
            print('[WARNING]: observation was clipped in Pendulum._get_observation().')
    ...
    def _create_urdf(self, file_name, length=None, inertia=None):
        ...
        if length is not None:
            ...
            out = root[1][0][0][0].attrib['size']
            out = ' '.join(out.split(' ')[:-1] + [str(2 * length)])
            root[1][0][0][0].attrib['size'] = out
            # Pole visual origin.
            out = root[1][0][1].attrib['xyz']
            out = ' '.join(out.split(' ')[:-1] + [str(length)])
            root[1][0][1].attrib['xyz'] = out
            # Pole inertial origin.
            out = root[1][1][0].attrib['xyz']
            out = ' '.join(out.split(' ')[:-1] + [str(length)])
            root[1][1][0].attrib['xyz'] = out
            # Pole inertia.
            root[1][1][2].attrib['ixx'] = str(inertia)
            root[1][1][2].attrib['iyy'] = str(inertia)
            root[1][1][2].attrib['izz'] = str(0.0)
            # Pole collision geometry box.
            out = root[1][2][0][0].attrib['size']
            out = ' '.join(out.split(' ')[:-1] + [str(2 * length)])
            root[1][2][0][0].attrib['size'] = out
            # Pole collision origin.
            out = root[1][2][1].attrib['xyz']
            out = ' '.join(out.split(' ')[:-1] + [str(length)])
            root[1][2][1].attrib['xyz'] = out
        return tree
