...
class QuadraticConstraint(Constraint):
    ...
    def __init__(
        ...
        assert P.shape == (self.dim, self.dim), (
            "[ERROR] P has the wrong dimension! It should match the dimension of"
...
class BoundedConstraint(LinearConstraint):
    ...
    def __init__(
        ...
        if active_dims is not None and isinstance(active_dims, list):
            assert self.lower_bounds.shape[0] == len(
                active_dims
            ), "[Error] active_dims and lower_bounds must have the same dimension."
        if active_dims is not None and isinstance(active_dims, int):
            assert (
                self.lower_bounds.shape[0] == 1
            ), "[Error] active_dims and lower_bounds must have the same dimension."
        if active_dims is not None and isinstance(active_dims, int):
            assert (
                self.upper_bounds.shape[0] == 1
            ), "[Error] active_dims and upper_bounds must have the same dimension."
...
class DefaultConstraint(BoundedConstraint):
    ...
    def __init__(
        ...
        if constrained_variable == ConstrainedVariableType.STATE:
            ...
            default_constraint_space = spaces.Box(
                low=env.physical_action_bounds[0],
...
class SymmetricStateConstraint(BoundedConstraint):
    ...
    def get_value(self, env):
        c_value = np.round(
            np.abs(self.constraint_filter @ env.state) - self.bound,
            decimals=self.decimals,
        )
        return c_value
