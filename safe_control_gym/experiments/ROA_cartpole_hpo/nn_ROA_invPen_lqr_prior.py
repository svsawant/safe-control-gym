...
L_p = eval(sys.argv[1]) # this will be 10
#################################### Constants ####################################
...
nn_roa_file_name = 'nn_roa_pendulum_' + str(L) + '.npy'
############################### System dynamics ################################
...
pendulum_p = InvertedPendulum(m, L_p, b, dt, [state_norm, action_norm])
A_p, B_p = pendulum_p.linearize()
dynamics_p = pendulum_p.__call__
...
L_dyn = lambda x: np.linalg.norm(A_p, 1) + np.linalg.norm(B_p, 1) * L_pol(x)
...
ax.set_title('ROA of pendulum under an LQR prior policy (l={:.2f})'.format(L_p))
...
# save lyapunov nn ROA
z = lyapunov_nn.safe_set.reshape(grid.num_points)
nn_roa_file_name = 'nn_roa_pendulum_' + str(L_p) + '.npy'
nn_roa_file_name = os.path.join(script_dir, nn_roa_file_name)
np.save(nn_roa_file_name, z)