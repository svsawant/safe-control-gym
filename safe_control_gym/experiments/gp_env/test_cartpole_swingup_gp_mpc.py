...
def make_plot(runs, traj, dir):
    ...
    for epoch in range(1, num_epochs):
        traj1 = np.vstack((runs['obs'][epoch][:, 0], runs['obs'][epoch][:, 2])).T
        np.savetxt(os.path.join(dir, 'traj_%s.csv' % epoch), traj1, delimiter=',')
        plt.plot(runs['obs'][epoch][:, 0], runs['obs'][epoch][:, 2], label='GP-MPC epoch %s' % epoch)
    # plt.plot(traj[:,0], traj[:,2], 'k',label='Reference')
    plt.legend()
    ...
    plt.legend()
    save_str = os.path.join(dir, 'cartpole_traj.png')
    plt.savefig(save_str)
...
if __name__ == "__main__":
    ...
    make_plot(test_data, ref ,config.output_dir)
