...
class ExperimentLogger:
    ...
    def __init__(
        self, log_dir, log_std_out=True, log_file_out=False, use_tensorboard=False
    ):
        """Initializes loggers.
    ...
    def load(self, step):
        ...
        if self.use_tensorboard:
            self.tb_logger = SummaryWriter(log_dir=self.log_dir, purge_step=step)
    ...
    def add_scalar(self, name, val, step, store=True, write=True, write_tb=True):
        """Logs a scalar data."""
    ...
    def add_scalars(
        self, data, step, prefix=None, store=True, write=True, write_tb=True
    ):
