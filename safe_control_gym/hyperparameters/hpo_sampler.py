...
GPMPC_dict = {
    "categorical": {
        ...
        "n_ind_points": [
            ...
            40,
            50,
        ],  # number should lower 0.8 * MIN(num_samples) if 0,2 is test_data_ratio
        ...
        "optimization_iterations": [
            ...
            3000,
            3200,
        ],  # to make sure having the same checkpoint at these steps [30000, 54000, 72000]
    },
    "float": {  # note that in float type, you must specify the upper and lower bound
...
def ppo_sampler(hps_dict: Dict[str, Any], trial: optuna.Trial) -> Dict[str, Any]:
    ...
    hidden_dim = trial.suggest_categorical(
        "hidden_dim", PPO_dict["categorical"]["hidden_dim"]
    )
    activation = trial.suggest_categorical(
        "activation", PPO_dict["categorical"]["activation"]
    )
    ...
    gae_lambda = trial.suggest_categorical(
        "gae_lambda", PPO_dict["categorical"]["gae_lambda"]
    )
    clip_param = trial.suggest_categorical(
        "clip_param", PPO_dict["categorical"]["clip_param"]
    )
    # Limit the KL divergence between updates
    target_kl = trial.suggest_float(
        "target_kl",
        PPO_dict["float"]["target_kl"][0],
        PPO_dict["float"]["target_kl"][1],
        log=True,
    )
    # Entropy coefficient for the loss calculation
    entropy_coef = trial.suggest_float(
        "entropy_coef",
        PPO_dict["float"]["entropy_coef"][0],
        PPO_dict["float"]["entropy_coef"][1],
        log=True,
    )

    # optim args
    opt_epochs = trial.suggest_categorical(
        "opt_epochs", PPO_dict["categorical"]["opt_epochs"]
    )
    mini_batch_size = trial.suggest_categorical(
        "mini_batch_size", PPO_dict["categorical"]["mini_batch_size"]
    )
    actor_lr = trial.suggest_float(
        "actor_lr",
        PPO_dict["float"]["actor_lr"][0],
        PPO_dict["float"]["actor_lr"][1],
        log=True,
    )
    critic_lr = trial.suggest_float(
        "critic_lr",
        PPO_dict["float"]["critic_lr"][0],
        PPO_dict["float"]["critic_lr"][1],
        log=True,
    )
    ...
    rollout_steps = trial.suggest_categorical(
        "rollout_steps", PPO_dict["categorical"]["rollout_steps"]
    )
    max_env_steps = trial.suggest_categorical(
        "max_env_steps", PPO_dict["categorical"]["max_env_steps"]
    )
    ...
    assert len(hps_suggestion) == len(hps_dict), ValueError(
        "We are optimizing over different number of HPs as you listed."
    )
...
def sac_sampler(hps_dict: Dict[str, Any], trial: optuna.Trial) -> Dict[str, Any]:
    ...
    # model args
    hidden_dim = trial.suggest_categorical(
        "hidden_dim", SAC_dict["categorical"]["hidden_dim"]
    )
    activation = trial.suggest_categorical(
        "activation", SAC_dict["categorical"]["activation"]
    )

    # loss args
    gamma = trial.suggest_categorical("gamma", SAC_dict["categorical"]["gamma"])
    tau = trial.suggest_float(
        "tau", SAC_dict["float"]["tau"][0], SAC_dict["float"]["tau"][1], log=False
    )

    # optim args
    train_interval = trial.suggest_categorical(
        "train_interval", SAC_dict["categorical"]["train_interval"]
    )
    train_batch_size = trial.suggest_categorical(
        "train_batch_size", SAC_dict["categorical"]["train_batch_size"]
    )
    actor_lr = trial.suggest_float(
        "actor_lr",
        SAC_dict["float"]["actor_lr"][0],
        SAC_dict["float"]["actor_lr"][1],
        log=True,
    )
    critic_lr = trial.suggest_float(
        "critic_lr",
        SAC_dict["float"]["critic_lr"][0],
        SAC_dict["float"]["critic_lr"][1],
        log=True,
    )

    max_env_steps = trial.suggest_categorical(
        "max_env_steps", SAC_dict["categorical"]["max_env_steps"]
    )
    warm_up_steps = trial.suggest_categorical(
        "warm_up_steps", SAC_dict["categorical"]["warm_up_steps"]
    )
...
def gpmpc_sampler(hps_dict: Dict[str, Any], trial: optuna.Trial) -> Dict[str, Any]:
    ...
    n_ind_points = trial.suggest_categorical(
        "n_ind_points", GPMPC_dict["categorical"]["n_ind_points"]
    )
    num_epochs = trial.suggest_categorical(
        "num_epochs", GPMPC_dict["categorical"]["num_epochs"]
    )
    num_samples = trial.suggest_categorical(
        "num_samples", GPMPC_dict["categorical"]["num_samples"]
    )

    # get dimensions of the dynamics
    d = len(hps_dict["learning_rate"])
    assert d == len(
        hps_dict["optimization_iterations"]
    ), "The number of optimization iterations must be the same as the number of learning rates."

    # use same setting for all dimensions for simplicity
    optimization_iterations, learning_rate = [], []

    optimization_iterations = d * [
        trial.suggest_categorical(
            "optimization_iterations",
            GPMPC_dict["categorical"]["optimization_iterations"],
        )
    ]
    learning_rate = d * [
        trial.suggest_float(
            "learning_rate",
            GPMPC_dict["float"]["learning_rate"][0],
            GPMPC_dict["float"]["learning_rate"][1],
            log=True,
        )
    ]
