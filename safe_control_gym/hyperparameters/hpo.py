...
class HPO(object):
    ...
    def __init__(
        ...
        **algo_config,
        ...
        assert len(hpo_config.objective) == len(
            hpo_config.direction
        ), "objective and direction must have the same length"
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        while increase_runs:
            ...
            for i in range(self.hpo_config.repetitions):
                ...
                try:
                    self.env_func = partial(
                        make, self.task, output_dir=self.output_dir, **self.task_config
                    )
                    ...
                    self.agent = make(
                        ...
                        checkpoint_path=os.path.join(
                            self.output_dir, "model_latest.pt"
                        ),
                ...
                _, metrics = experiment.run_evaluation(
                    n_episodes=5, n_steps=None, done_on_max_steps=True
                )
            ...
            if (
                self.hpo_config.warm_trials < len(self.study.trials)
                and self.hpo_config.dynamical_runs
            ):
    ...
    def hyperparameter_optimization(self) -> None:
        ...
        self.study.optimize(
            ...
            callbacks=[
                MaxTrialsCallback(self.hpo_config.trials, states=(TrialState.COMPLETE,))
            ],
        )
        ...
        if len(self.hpo_config.direction) == 1:
            ...
            for i in range(
                min(self.hpo_config.save_n_best_hps, len(self.study.trials))
            ):
                params = trials[i].params
                with open(
                    f"{output_dir}/hyperparameters_{trials[i].value:.4f}.yaml", "w"
                ) as f:
            ...
            for i in range(len(self.study.best_trials)):
                ...
                with open(
                    f"{output_dir}/best_hyperparameters_[{best_trials[i].values[0]:.4f},{best_trials[i].values[1]:.4f}].yaml",
                    "w",
                ) as f:
                    yaml.dump(params, f, default_flow_style=False)
        ...
        try:
            if len(self.hpo_config.objective) == 1:
                ...
                for i in range(len(self.hpo_config.objective)):
                    ...
                    plt.savefig(
                        output_dir
                        + f"/param_importances_{self.hpo_config.objective[i]}.png"
                    )
                    # plt.show()
                    plt.close()
                    ...
                    plt.savefig(
                        output_dir
                        + f"/optimization_history_{self.hpo_config.objective[i]}.png"
                    )
                    # plt.show()
                    plt.close()
        except Exception as e:
